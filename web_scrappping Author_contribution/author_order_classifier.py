...
# ── ACI & Credit Distribution Helpers ───────────────────────────────────────

def calculate_aci(contributions: Dict[str, float]) -> Dict[str, float]:
    """
    Calculate Author Contribution Index (ACI) for each author.

    ACI_i = C_i / (sum(C_j) - C_i)
    """
    total = sum(contributions.values())
    if total == 0:
        return {author: 0.0 for author in contributions}
    ...
    for author, contribution in contributions.items():
        ...
        denominator = total - contribution
        aci_scores[author] = (
            float("inf") if denominator == 0
            else round(contribution / denominator, 4)
        )

    return aci_scores


def distribute_credit_contribution_based(authors: List[str], n_authors: int) -> Dict[str, float]:
    """Distribute credit using a position-weighted contribution model."""
    if n_authors == 1:
    ...
    for index, author in enumerate(authors):
        if index < 3:
            credit_dist[author] = weights[index] * 100
        else:
            credit_dist[author] = 20.0 / (n_authors - 3)

    total = sum(credit_dist.values())
    return {
        author: round(credit * 100 / total, 2)
        for author, credit in credit_dist.items()
    }


def distribute_credit_alphabetical(
    authors: List[str],
    n_authors: int,
    contributions: Optional[Dict[str, float]] = None,
) -> Dict[str, float]:
    """Distribute credit equally, or by ACI when contributions are declared."""
    ...
    total_aci = sum(score for score in aci_scores.values() if score != float("inf"))

    if total_aci == 0:
        return {author: equal_share for author in authors}

    return {
        author: round((aci_scores[author] / total_aci) * 100, 2)
        if author in aci_scores and aci_scores[author] != float("inf")
        else equal_share
        for author in authors
    }


def distribute_credit_mixed(
    authors: List[str],
    n_authors: int,
    contributions: Optional[Dict[str, float]] = None,
) -> Dict[str, float]:
    """Use ACI when available for mixed ordering; otherwise use equal credit."""
    return distribute_credit_alphabetical(authors, n_authors, contributions)
